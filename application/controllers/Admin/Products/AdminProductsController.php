	...
	public function create()
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if(isset($inputs['variations'])) { 
				$variations_count = count($inputs['variations']);
				if($variations_count > 0){
					$inputs['variations']  = serialize($this->input->post('variations'));
				} 
			} 
...
//				}
		...
		$data['attributes'] = $this->AttributesModel->getAttributesDropdown();
	...
	public function edit($id)
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if ($this->form_validation->run()) {
                ...
                if (!empty($_FILES['cover_image']['name']) && $_FILES['cover_image']['name'] != null) {
				//Upload single file. i.e Product Cover image.
				$cover_image = $this->uploadFile('cover_image', $this->getUploadConfig());
				//Create Product thumbnail
				$this->createThumb($cover_image);
