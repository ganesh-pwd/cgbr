...
class WishListController extends CI_Controller {
    ...
    /**
     * Check product to user's wishlist
     */
    ...
    /**
     * delete product to user's wishlist
     */
    public function deleteToWishList()
    {
        ...
        {
            $status = $this->UserWishListModel->delete($this->input->post('wishlist_id'));
            if($status){
                $this->session->set_flashdata('info', 'Wishlist deleted successfully.');
            }
            else{
                $this->session->set_flashdata('error', 'Failed to delete Wishlist product.');
            }
